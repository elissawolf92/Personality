...
public class LMAPlayerGUIMecanim : MonoBehaviour {
	...
	private string[] _animDisplayNameStr = { "Pointing", "Picking up a pillow", "Lifting"};
	...
	void Start(){
		...
		//_agent = GameObject.Find ("ArabMale");
	...
	public void Reset() {
		ChangeAnimation ("Pointing_to_Spot_Netural_02_Updated");
		_animInd = 0;
		UpdateParameters();
	...
	void Update(){
		...
		if (_animInd != comboBoxControl.SelectedItemIndex) {
			...
			//_agent.GetComponent<ArmAnimator>().changeAnim(_animNames[_animInd]);
		} 
...
//			_agent.GetComponent<TorsoController>().Reset();
	...
	*/
	...
	void ChangeAnimation(string animName) {
		_agent.GetComponent<AnimationInfo>().Reset(animName);     
		//_agent.GetComponent<ArmAnimator> ().changeAnim (animName);
		//_agent.GetComponent<ArmAnimator>().Reset();
		//_agent.GetComponent<TorsoAnimator> ().Reset (animName);
		//_agent.GetComponent<TorsoController> ().Reset (animName);
		_agent.animation.enabled = true; 
	}
	
	// Play the animation that is currently selected
	void PlayAnim(GameObject agent) {

		Animation a = agent.GetComponent<Animator> ().animation;
		if (a == null)
						Debug.Log ("Damn.");
				else
						Debug.Log ("yay?");
		...
		string stateName = _animNames [_animInd];
