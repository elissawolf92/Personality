...
public class MotionAnalyzer : IMotionAnalyzer {
	...
	private float FindContactTime(LegCycleData data, bool useToe, int searchDirection, float yRange, float threshold) {
		...
		for (int i=0; i<samples && i>-samples; i+=searchDirection) {
			...
			for (int s=0; s<3; s++) {
				j[s] = LocomotionUtil.Mod(i+data.stanceIndex-spread+spread*s,samples);
		...
		return GetTimeFromIndex(LocomotionUtil.Mod(curvatureMaxIndex-data.stanceIndex,samples));
	}
	...
	private float FindSwingChangeTime(LegCycleData data, int searchDirection, float threshold) {
		...
		for (int i=0; i<samples && i>-samples; i+=searchDirection) {
			...
			if (Mathf.Abs((currentSpeed-stanceSpeed)/stanceSpeed)>threshold) {
				return GetTimeFromIndex(LocomotionUtil.Mod(j[1]-data.stanceIndex,samples));
			}
		}
		return LocomotionUtil.Mod(searchDirection*-0.01f);
	}
	...
	private int GetIndexFromTime(float time) {
		return LocomotionUtil.Mod((int)(time*samples+0.5),samples);
	}
	...
	private Vector3 GetVector3AtTime(int leg, float flightTime, GetVector3Member get) {
		...
		index = LocomotionUtil.Mod(index+cycles[leg].stanceIndex,samples);
		return (
			get(cycles[leg].samples[index])*(1-weight)
			+get(cycles[leg].samples[LocomotionUtil.Mod(index+1,samples)])*(weight)
		);
	}
	...
	public override Vector3 GetFlightFootPosition(int leg, float flightTime, int phase) {
		...
		return (
			...
			+ cycles[leg].samples[LocomotionUtil.Mod(index+1,samples)].footBaseNormalized * (weight)
		);
	}
	...
	private void FindCycleAxis(int leg) {
		...
		for (int i=0; i<samples; i++) {
			...
			cycles[leg].cycleCenter += LocomotionUtil.ProjectOntoPlane(s.middle, Vector3.up);
		}
		...
		for (int i=0; i<samples; i++) {
			...
			Vector3 curvePoint = LocomotionUtil.ProjectOntoPlane(s.middle, Vector3.up);
		...
		for (int leg=0; leg<legs; leg++) {
			...
			if (motionType==MotionType.WalkCycle) {
				...
				for (int i=0; i<samples+1; i++) {
					...
					+Mathf.Abs(
						LocomotionUtil.ProjectOntoPlane(s.middle-cycles[leg].cycleCenter, Vector3.up).magnitude
			...
			cycles[leg].heelToetipVector = LocomotionUtil.ProjectOntoPlane(cycles[leg].heelToetipVector, Vector3.up);
			...
			if (motionType==MotionType.WalkCycle) {
				...
				Vector3 stanceSlideVector = (
					cycles[leg].samples[GetIndexFromTime(LocomotionUtil.Mod(cycles[leg].liftoffTime+cycles[leg].stanceTime))].footBase
					-cycles[leg].samples[GetIndexFromTime(LocomotionUtil.Mod(cycles[leg].strikeTime+cycles[leg].stanceTime))].footBase
				);
				// FIXME: Assumes horizontal ground plane
		...
		for (int leg=0; leg<legs; leg++) {
			if (motionType==MotionType.WalkCycle) {
				for (int j=0; j<samples; j++) {
					int i = LocomotionUtil.Mod(j+cycles[leg].stanceIndex,samples);
					LegCycleSample s = cycles[leg].samples[i];
	...
	public void RenderGraphAll(MotionAnalyzerDrawOptions opt, int currentLeg) {
		...
		// Key positions
		Vector3 footHeel = ankleT.position+LocomotionUtil.TransformVector(
			ankleT,legC.legs[l].ankleHeelVector
		);
		Vector3 footToetip = toeT.position+LocomotionUtil.TransformVector(
		...
		if (opt.drawFootPrints) {
			float cycleTime = LocomotionUtil.Mod(time-cycles[l].stanceTime+cycleOffset);
			...
			Vector3 pos = (
				...
				+(cycleDirection*-cycleDistance*(LocomotionUtil.Mod(cycleTime+0.5f,1)-0.5f))
			);
		...
		if (opt.drawTrajectories) {
			...
			for (int i=0; i<samples*2; i++) {
				int j = LocomotionUtil.Mod(cycleIndex-i,samples);
				LegCycleSample sA = cycles[l].samples[j];
				LegCycleSample sB = cycles[l].samples[LocomotionUtil.Mod(j-1,samples)];
				...
				if (opt.drawTrajectoriesProjected) {
					...
					DrawLine (
						objectMatrix.MultiplyPoint3x4(
							LocomotionUtil.ProjectOntoPlane(sA.heel+sA.toetip,Vector3.up)/2
							+ (legC.groundPlaneHeight-graphSpan.y)*Vector3.up
						),
						objectMatrix.MultiplyPoint3x4(
							LocomotionUtil.ProjectOntoPlane(sB.heel+sB.toetip,Vector3.up)/2
							+ (legC.groundPlaneHeight-graphSpan.y)*Vector3.up
						),
						strongColor
					);
		...
		if (opt.drawGraph) {
			...
			DrawArea graphZ = new DrawArea3D(
				...
				LocomotionUtil.CreateMatrix(sideDir, upDir*opt.graphScaleV, cycleDir*opt.graphScaleH, groundPos)
			);
			...
			for (int i=0; i<2; i++) {
				...
				if (opt.drawStanceMarkers) {
					graphZ.DrawRect(
						new Vector3(LocomotionUtil.Mod(timeRounded-cycles[l].stanceTime+cycleOffset,1)+i,0,graphMax.z),
						Vector3.up*graphSpan.y,
						-Vector3.forward*(graphSpan.z+normExtend),
						strongColor
					);
			...
			if (opt.drawHeelToe) {
				DrawLine(
					ankleT.position+LocomotionUtil.TransformVector(ankleT,legC.legs[l].ankleHeelVector),
				...
				DrawLine(
					toeT.position+LocomotionUtil.TransformVector(toeT,legC.legs[l].toeToetipVector),
					graphZ.Point(new Vector3(
						0,
			...
			for (int i=0; i<samples*2; i++) {
				...
				float t1 = GetTimeFromIndex(i  );
				float t2 = GetTimeFromIndex(i+1);
				float c1 = GetTimeFromIndex(LocomotionUtil.Mod(j  -cycles[l].stanceIndex,samples));
				float c2 = GetTimeFromIndex(LocomotionUtil.Mod(j-1-cycles[l].stanceIndex,samples));
