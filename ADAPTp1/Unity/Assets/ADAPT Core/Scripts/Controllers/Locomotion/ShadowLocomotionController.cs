/*
Copyright (c) 2010, Rune Skovbo Joxfghhansen & Unity Technologies ApS
...
/// <summary>
/// Version of the Unity LegAnimator script for use with the shadow animation system.
/// Should be the SAME as the the original LegAnimator script, except for changes made 
/// to make it work with the shadow system (although some small changes may be included
/// just as a result of me playing with it).
/// 
/// Key changes made have been labeled with comments prepended with "NATHAN:"
/// 
/// See http://na-3d-attack.blogspot.com/2012/07/how-easy-is-it-to-make-shadow-version.html for 
/// some notes on difficulty in integrating the LegAnimator into the Shadow Animation System.
/// 
/// \warning There is a dependency on the original LegController script for data structures
/// such as LegInfo and MotionGroupInfo.
/// 
/// (Original LegAnimator Copyright (c) 2010, Rune Skovbo Johansen & Unity Technologies ApS
/// See the document "TERMS OF USE" included in the project folder for licencing details.)
/// </summary>
[RequireComponent(typeof(LegController))]
[RequireComponent(typeof(AlignmentTracker))]
public class ShadowLocomotionController : ShadowController
{
    /// <summary>
    /// We must give the LegAnimator the root of the 
    /// </summary>
    ...
    // Model dependent, thus no better default
    public float minStepDistance = 0.2f;
    // Sensible for most models
    public float maxStepDuration = 1.5f;
    // Sensible for most models, must be less than 180
    public float maxStepRotation = 160;
    // Model dependent, thus no better default
    public float maxStepAcceleration = 5.0f;
    public float maxStepHeight = 1.0f;
    // Sensible for most models, must be less than 90
    ...
    // Default layer per default
    public LayerMask groundLayers = 1;
    public Terrain overrideGround;
    ...
    public bool renderLegBones = false;
    ...
    {
        if (!LocomotionUtil.IsSaneNumber(f)) UnityEngine.Debug.LogError(text + "=" + f);
    }

    [Conditional("DEBUG")]
    void AssertSane(Vector3 vect, string text)
    {
        if (!LocomotionUtil.IsSaneNumber(vect.x)
            || !LocomotionUtil.IsSaneNumber(vect.y)
            || !LocomotionUtil.IsSaneNumber(vect.z)
    ...
    {
        if (!LocomotionUtil.IsSaneNumber(q.x)
            || !LocomotionUtil.IsSaneNumber(q.y)
            || !LocomotionUtil.IsSaneNumber(q.z)
            || !LocomotionUtil.IsSaneNumber(q.w)
        ) UnityEngine.Debug.LogError(text + "=" + q);
    }

    bool stupidRaycasts = false;
    ...
    void OnEnable()
    {
        // TODO: Can this be moved to Awake()? - AS
        updateStates = true;
        if (legC == null) return;
        ResetMotionStates();
        ResetSteps();
    ...
    /// <summary>
    /// Same (I think) as the old ResetMotionStates() function, except 
    /// instead of using this.anmiation, we use this.transform.animation. 
    /// this.transform is the gameObject that is at the root of the shadow 
    /// hierarchy.
    /// </summary>
    private void ResetMotionStates()
    {
        ...
        for (int g = 0; g < motionGroupStates.Length; g++)
        {
            ...
            motionGroupStates[g].motionStates = 
                new AnimationState[legC.motionGroups[g].motions.Length];
            motionGroupStates[g].relativeWeights = 
                new float[legC.motionGroups[g].motions.Length];
    ...
    /// <summary>
    /// Same (I think) as the original ResetSteps() function, but with 
    /// references to this.transform.up. changed to this.transform.up. 
    /// this.transform is the gameObject that is at the root
        ...
        for (int leg = 0; leg < legs.Length; leg++)
        {
            ...
            legStates[leg].stepFromMatrix = FindGroundedBase(
                this.transform.TransformPoint(
                    legStates[leg].stancePosition / scale),
                this.transform.rotation,
                legStates[leg].heelToetipVector,
                false
            );
            legStates[leg].stepFromPosition = 
                legStates[leg].stepFromMatrix.GetColumn(3);
    ...
    /// <summary>
    /// Same (I think) as the original Update() function, but with references
    /// to this.transform.up. changed to this.transform.up. this.transform is 
    /// the gameObject that is at the root of the shadow hierarchy.
    /// </summary>
    public override void ControlledUpdate()
    {

        if (Time.deltaTime == 0 || Time.timeScale == 0) return;
        scale = this.transform.lossyScale.z;

        AssertSane(tr.velocity, "tr.velocity");
        AssertSane(up, "up");

        // When calculating speed, clamp vertical speed to be no longer 
        // than horizontal speed to avoid sudden spikes when 
        // CharacterController walks up a step.
        Vector3 velocityVClamped = LocomotionUtil.ProjectOntoPlane(tr.velocity, up);
        speed = velocityVClamped.magnitude;
        velocityVClamped = 
            velocityVClamped 
            + up * Mathf.Clamp(Vector3.Dot(tr.velocity, up), -speed, speed);
        speed = velocityVClamped.magnitude;

        hSpeedSmoothed = 
            LocomotionUtil.ProjectOntoPlane(tr.velocitySmoothed, up).magnitude;

        objectVelocity =
            this.transform.InverseTransformPoint(tr.velocitySmoothed)
            - this.transform.InverseTransformPoint(Vector3.zero);

        // Check if velocity (and turning - not implemented yet) have changed 
        // significantly
        bool newVelocity = false;

        float magnitude = (objectVelocity - usedObjectVelocity).magnitude;
        float threshhold = 
            0.002f 
            * Mathf.Min(
                objectVelocity.magnitude, 
                usedObjectVelocity.magnitude);

        if (magnitude > threshhold || updateStates)
        {
            newVelocity = true;
        ...
        for (int g = 0; g < legC.motionGroups.Length; g++)
        {
            ...
            if (group.controller.enabled == false 
                || newGroupWeight < smallWeightDifference) newGroupWeight = 0;
            else if (newGroupWeight > 1 - smallWeightDifference) 
                newGroupWeight = 1;

            float weightDiff = Mathf.Abs(newGroupWeight - group.weight);
            if (weightDiff > smallWeightDifference)
            {
                changedGroupWeight = true;
                newWeights = true;
                if (group.weight == 0 && newGroupWeight > 0) 
                    justEnabled = true;
                group.weight = newGroupWeight;
            }

            // Check if primary weight in motion group have changed 
            // significantly by external factors, for example a CrossFade
            // that fades down these weights. We must then enforce that the 
            // weights are again set according to the group dictate.
            else if (
                Mathf.Abs(
        ...
        if (newWeights || updateStates)
        {
            ...
            if (summedCycleMotionWeight > 0)
            {
                for (int leg = 0; leg < legs.Length; leg++)
                {
                    ...
                    legStates[leg].stanceTime = LocomotionUtil.Mod(
        ...
        float groundAccelerationMagnitude = LocomotionUtil.ProjectOntoPlane(tr.accelerationSmoothed, up).magnitude;
        ...
        if (!allParked)
        {
            normalizedTime = LocomotionUtil.Mod(normalizedTime + (1 / cycleDuration) * Time.deltaTime);
            for (int m = 0; m < legC.cycleMotions.Length; m++)
            {
    ...
    public override void ControlledLateUpdate()
    {
        if (Time.deltaTime == 0 || Time.timeScale == 0) return;
        ...
        for (int leg = 0; leg < legs.Length; leg++)
        {
            ...
            float designatedCycleTime = LocomotionUtil.CyclicDiff(normalizedTime, legStates[leg].stanceTime);
            ...
            if (legStates[leg].cycleTime < legStates[leg].strikeTime)
            {
                ...
                {
                    Quaternion oldPredictedRotation = LocomotionUtil.QuaternionFromMatrix(legStates[leg].stepToMatrix);
                    ...
                    predictedRotation = LocomotionUtil.ConstantSlerp(
                ...
                newStepPosition = LocomotionUtil.SetHeight(
            ...
            if (enableLegParking)
            {
                ...
                float distToNextStep = LocomotionUtil.ProjectOntoPlane(
        ...
        for (int leg = 0; leg < legs.Length; leg++)
        {
            ...
            Vector3 stepBodyPoint = this.transform.TransformDirection(-legStates[leg].stancePosition) * scale;
        ...
        for (int leg = 0; leg < legs.Length; leg++)
        {
            ...
            legStates[leg].footBase += LocomotionUtil.ProjectOntoPlane(offset * flightProgressionLift, legsUp);
            ...
            Quaternion footBaseRotationFromSteps = Quaternion.Slerp(
                LocomotionUtil.QuaternionFromMatrix(legStates[leg].stepFromMatrix),
                LocomotionUtil.QuaternionFromMatrix(legStates[leg].stepToMatrix),
                flightTime
            );
            ...
            Vector3 footBaseElevated = Vector3.Lerp(
                ...
                LocomotionUtil.SetHeight(legStates[leg].footBase, groundBasePoint, legsUp),
                flightTimeLift
            );
        ...
        legC.root.transform.rotation = (
            tr.rotation * Quaternion.Inverse(this.transform.rotation)
            * bodyRotation
            * legC.root.transform.rotation
        );
        for (int leg = 0; leg < legs.Length; leg++)
        ...
        Vector3 hipAverage = this.transform.TransformPoint(legC.hipAverage);
        Vector3 hipAverageGround = this.transform.TransformPoint(legC.hipAverageGround);
        ...
        legC.root.transform.position = rootPointAdjusted + position - this.transform.position;
    ...
    private bool ModifiedRaycast(Vector3 pos, out RaycastHit hit)
    {
        hit = new RaycastHit();

        if (stupidRaycasts == true)
        {
            hit.point = pos;
        ...
        else if (overrideGround != null)
        {
            if (overrideGround.collider.bounds.Contains(pos))
            {
                ...
                float height = localHeight + overrideGround.transform.position.y;
                if (height < pos.y + maxStepHeight && height > pos.y - maxStepHeight)
                {
                    Vector3 hitPos = pos;
                    hitPos.y = height;
                    hit.point = hitPos;
                    hit.normal = overrideGround.terrainData.GetInterpolatedNormal(terrainPos.x, terrainPos.z);
                    return true;
                }
            }
            return false;
        ...
        {
            return Physics.Raycast(pos + up * maxStepHeight, -up, out hit, maxStepHeight * 2, groundLayers);
        }
    }
    ...
    )
    {
        ...
        if (ModifiedRaycast(pos, out hit))
        {
            valid = true;
            hitAPoint = hit.point;
        ...
        if (ModifiedRaycast(pos + heelToToetip, out hit))
        {
            valid = true;
            hitBPoint = hit.point;
            // Ignore surface normal if it deviates too much
        ...
        return LocomotionUtil.MatrixFromQuaternionPosition(rot, newStepPosition);
    }
    ...
    public void FindHipOffset()
    {
        ...
        for (int leg = 0; leg < legs.Length; leg++)
        {
            ...
            float desiredDistanceGround = LocomotionUtil.ProjectOntoPlane(desiredVector, legsUp).magnitude;

            // Move closer if too far away
            Vector3 ankleVectorGround = LocomotionUtil.ProjectOntoPlane(
            ...
            intersections = LocomotionUtil.GetLineSphereIntersections(
                Vector3.zero, legsUp,
    ...
    public void AdjustLeg(int leg, Vector3 desiredAnklePosition, bool secondPass)
    {
        ...
        if (!secondPass)
        {
            ...
            Quaternion horizontalRotation = Quaternion.FromToRotation(
                forward,
                LocomotionUtil.ProjectOntoPlane(legStates[leg].footBaseRotation * Vector3.forward, up)
        ...
        else
        {
            ...
            Quaternion legAxisRotate = Quaternion.Slerp(
                ...
                Quaternion.FromToRotation(
                    LocomotionUtil.ProjectOntoPlane(forward, hipAnkleVector),
                    LocomotionUtil.ProjectOntoPlane(legStates[leg].footBaseRotation * Vector3.forward, hipAnkleVector)
                ),
                0.5f
            );
    ...
    /// <summary>
    /// Same (I think) as the original RenderBlendingGraph() function, but with references to this
    /// changed to this.transform.up. this.transform is the gameObject that is at the root
    /// of the shadow hierarchy.
    /// </summary>
    public void RenderBlendingGraph()
    {
        Matrix4x4 matrix = LocomotionUtil.CreateMatrix(
            this.transform.right, this.transform.forward, this.transform.up,
            this.transform.TransformPoint(legC.hipAverage)
        );
        float size = (Camera.main.transform.position - this.transform.TransformPoint(legC.hipAverage)).magnitude / 2;
    ...
    public void RenderCycleGraph()
    {
        ...
        for (int leg = 0; leg < legs.Length; leg++)
        {
            ...
            float dictatedCycle = LocomotionUtil.CyclicDiff(normalizedTime, legStates[leg].stanceTime);
